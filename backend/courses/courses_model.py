...
def get_course_students(user_id, course_id):
    ...
    try:
        ...
        rows = conn.execute('''
            SELECT ce.id, ce.user_id, ce.status, ce.payment_status, ce.payment_screenshot, ce.enrolled_at,
...
def review_course_enrollment(user_id, enrollment_id, status, message=''):
    conn = get_db_connection()
    try:
        ...
        row = conn.execute('SELECT id FROM course_enrollments WHERE id = ?', (enrollment_id,)).fetchone()
        if not row:
            return False, "Enrollment not found", 404

        payment_status = 'paid' if status == 'approved' else 'pending'
        conn.execute(
            'UPDATE course_enrollments SET status = ?, payment_status = ? WHERE id = ?',
            (status, payment_status, enrollment_id)
        )
        conn.commit()
        return True, "Enrollment updated successfully", 200
    except Exception as e:
        conn.rollback()
        raise e
    finally:
