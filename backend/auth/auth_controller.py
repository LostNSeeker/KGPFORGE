...
def _extract_user_id_from_identity(identity):
    if isinstance(identity, str) and identity.startswith('user_'):
        return int(identity.split('_', 1)[1])
    return int(identity)
...
@jwt_required()
def me():
    """Fetch the latest authenticated user data from DB."""
    try:
        identity = get_jwt_identity()
        user_id = _extract_user_id_from_identity(identity)
        user = get_user_by_id(user_id)
        if not user:
            return jsonify({'error': 'User not found'}), 404

        user.pop('password_hash', None)
        user['is_approved'] = bool(user.get('is_approved'))
