...
@launchpad_bp.route('/launchpad/upload/service-image', methods=['POST'])
@jwt_required()
def upload_service_image():
    ...
    filename = f"service_{uuid.uuid4().hex}.{ext}"
    file.save(os.path.join(current_app.config['UPLOAD_FOLDER'], filename))
    return jsonify({'filename': filename, 'url': f"/api/profile/picture/{filename}"}), 200
...
@jwt_required()
def admin_update_timeline(item_id):
    if not launchpad_model.is_admin(get_user_id_from_jwt()): return jsonify({'error': 'Admin required'}), 403
    try:
