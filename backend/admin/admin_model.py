...
def get_admin_stats(user_id):
    ...
    try:
        ...
        founders_count = conn.execute("SELECT COUNT(*) FROM users WHERE role = 'founder'").fetchone()[0]
        alumni_count = conn.execute("SELECT COUNT(*) FROM users WHERE role = 'alumni'").fetchone()[0]
        mentors_count = conn.execute("SELECT COUNT(*) FROM users WHERE role = 'mentor'").fetchone()[0]
        investors_count = conn.execute("SELECT COUNT(*) FROM users WHERE role = 'investor'").fetchone()[0]
        courses_count = conn.execute("SELECT COUNT(*) FROM courses").fetchone()[0]
        services_count = conn.execute("SELECT COUNT(*) FROM services WHERE is_active = 1").fetchone()[0]
        events_count = conn.execute("SELECT COUNT(*) FROM events").fetchone()[0]

        stats = [
            {'key': 'users_count', 'label': 'Users', 'value': str(users_count)},
            {'key': 'founders_count', 'label': 'Founders', 'value': str(founders_count)},
            {'key': 'alumni_count', 'label': 'Alumni', 'value': str(alumni_count)},
            {'key': 'students_count', 'label': 'Students', 'value': str(students_count)},
            {'key': 'courses_count', 'label': 'Courses', 'value': str(courses_count)},
            {'key': 'services_count', 'label': 'Active Services', 'value': str(services_count)},
            {'key': 'events_count', 'label': 'Events', 'value': str(events_count)},
            {'key': 'mentors_count', 'label': 'Mentors', 'value': str(mentors_count)},
            {'key': 'investors_count', 'label': 'Investors', 'value': str(investors_count)},
        ]
